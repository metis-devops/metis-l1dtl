...
func (m *mockEth) GetBlockByNumber(n string, _ bool) (*types.Header, error) {
	if n == "latest" {
		return m.headers[2], nil
	}
	i, e := hexutil.DecodeUint64(n)
	if e != nil || i >= uint64(len(m.headers)) {
		return nil, e
	}
	return m.headers[i], nil
}
...
func TestServiceProcess(t *testing.T) {
	...
	for n := range int64(3) {
		h := &types.Header{Number: big.NewInt(n), Difficulty: big.NewInt(0), ParentHash: parent, Time: uint64(100 + n)}
